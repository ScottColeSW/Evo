...
class Simulation:
    ...
    async def _safe_llm_result(self, coro, label: str) -> dict:
        """Runs one of this file's many "resolve a pending X with a real,
        unbatched LLM call" coroutines (elect_chief, breed_individuals, hatch,
        name_settlement, breed, reflect_on_history), catching any failure
        instead of letting it propagate.

        Live report, 2026-09-19: confirmed via the new _raise_with_body fix
        (ollama_client.py) that a real 500 -- Ollama's own "prediction
        aborted, token repeat limit reached" safety valve -- can fire from
        any of these. ModelBatchScheduler.run_batch already isolates this
        exact failure for ordinary per-turn decisions (asyncio.gather(...,
        return_exceptions=True), "so one bad turn can't take down the whole
        tick" -- see its own docstring); these six calls had no equivalent
        protection at all. Uncaught, the exception propagates all the way up
        through step() to app.py._tick_session's outer catch-all, which skips
        the ENTIRE tick for EVERY tribe in the session -- not just the one
        resolution that actually failed. Every one of these six callers
        already has its own graceful fallback for an empty/unusable result
        (e.g. _resolve_hatch's trait falls back to "unremarkable but hardy"),
        so returning {} here just routes a real failure through logic that's
        already tested and correct, rather than duplicating fallback text in
        six places."""
        try:
            return await coro
        except Exception:
    ...
    async def _install_chief(self, tribe: "Tribe") -> None:
        ...
        result = await self._safe_llm_result(
            elect_chief(self.client, tribe.model, tribe.name, water_needed, context), "elect_chief"
        )
    ...
    async def _resolve_birth(self, tribe: "Tribe") -> None:
        ...
        result = await self._safe_llm_result(
            breed_individuals(self.client, tribe.model, tribe.name, parent_a, parent_b), "breed_individuals"
        )
    ...
    async def _resolve_hatch(self, tribe: "Tribe") -> None:
        ...
        if not parents:
            ...
            result = await self._safe_llm_result(
                hatch(self.client, tribe.model, parents[0], parents[1], era_label), "hatch"
            )
        else:
    ...
    async def _resolve_settlement_naming(self, tribe: "Tribe") -> None:
        ...
        result = await self._safe_llm_result(
            name_settlement(self.client, tribe.model, tribe.name, tribe.chief_name, biome), "name_settlement"
        )
    ...
    async def _resolve_cultural_crossover(self, tribe: "Tribe") -> None:
        ...
        era_label = next((e.label for e in ERAS if e.key == tribe.era), tribe.era)
        result = await self._safe_llm_result(
            breed(self.client, tribe.model, tribe_a, tribe_b, era_label), "breed"
        )
