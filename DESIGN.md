...
Memory scores relevance by token overlap (Jaccard) between a query and stored episodes, not embeddings — cruder, but it actually correlates with what the text is about. Entries are tagged by kind (ordinary episodes vs. a chief's own private reflections) so the two can never surface in each other's place just because they share a few words. High-weight episode entries periodically condense into permanent taboos; a reflection never does — self-knowledge isn't a danger to avoid the way a taboo is. The emotional matrix independently imprints pride or dread around locations and can increase inference temperature during distress.

Leadership begins with an LLM-generated contest and philosophy. A periodic night cycle, run by a dedicated reviewer model (not the tribe's own live model, so the chief has a real outside voice to think against — see [`config.REFLECTION_MODEL`](backend/config.py)), first reflects freely on recent events — collapsing repeated event types to the single latest instance, so an automatic, frequent event like an egg hatch never crowds out everything else worth reflecting on — then, only if that thinking leads somewhere concrete, may revise its philosophy, propose an honor, or set a standing decree. A private thought is remembered the same way any other episode is; one that recurs across several nights earns real behavioral pull of its own, promoted straight into the standing decree once it's reinforced enough times — but only into an empty slot, never overriding a decree the chief set explicitly.
